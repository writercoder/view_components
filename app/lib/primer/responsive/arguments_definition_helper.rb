...
module Primer
  module Responsive
    ...
    module ArgumentsDefinitionHelper
      ...
      # "flag" a argument definition hash to be created as a proper ArgumentDefinition
      # allowing for argument namespacing
      def prop(argument_definition = {})
      ...
      def validate_argument_values(arguments_definition:, argument_values: {})
        arguments_definition.each do |prop, possible_definition|
          unless possible_definition.is_a? ArgumentDefinition
            validate_argument_values(
              arguments_definition: possible_definition,
              argument_values: argument_values.fetch(prop, {})
            )
            next
          end

          definition = possible_definition
          if definition.deprecated_value?(argument_values[prop])
            definition.deprecation.deprecation_warn(argument_values[prop])
            next
          end

          if definition.required? && !definition.responsive?(:yes)
            raise InvalidArgumentValueError, <<~MSG
              #{definition.invalid_value_base_message(argument_values[prop])}
              Argument value is required.
            MSG
          end

          # if definition.responsive == :yes && argument_values.key?(prop)
          #   raise InvalidArgumentValueError, <<~MSG
          #     #{definition.invalid_value_base_message(argument_values[prop])}
          ...
          definition.validate_value(argument_values[prop]) if argument_values.key?(prop)
          RESPONSIVE_VARIANTS.each do |variant|
            next unless argument_values.key?(variant)

            definition.validate_value(argument_values[variant][prop], variant)
          end
        end
      end
      ...
      def fill_missing_values_with_default(arguments_definition:, argument_values: {}, fallback_to_default: false)
        arguments_definition.each do |prop, possible_definition|
          unless possible_definition.is_a? ArgumentDefinition
            argument_values[prop] = fill_missing_values_with_default(
              arguments_definition: possible_definition,
              argument_values: argument_values[prop] || {},
              fallback_to_default: fallback_to_default
            )
            next unless argument_values[prop].is_a? Hash
            ...
            # the "root" level hash
            RESPONSIVE_VARIANTS.each do |variant|
              next unless argument_values[prop].key? variant

              argument_values[variant] = {} unless argument_values.key? variant
              argument_values[variant][prop] = argument_values[prop][variant]
            end

            # cleanup
            argument_values[prop].except!(*RESPONSIVE_VARIANTS)
            argument_values.delete(prop) if argument_values[prop].empty?
            next
          end

          definition = possible_definition
          has_to_fallback = fallback_to_default && !definition.valid_value?(argument_values[prop])
          next unless !argument_values.key?(prop) || argument_values[prop].nil? || has_to_fallback

          if definition.responsive?
            base_value = argument_values[prop] if argument_values.key? prop

            if definition.responsive?(:transitional) && !base_value.nil?
              argument_values[prop] = definition.default_value
              next
            end
            ...
            RESPONSIVE_VARIANTS_MAP.each do |responsive_variant, responsive_variant_config|
              ...
              has_defined_variant = argument_values[responsive_variant].key?(prop)
              responsive_value = has_defined_variant ? argument_values[responsive_variant][prop] : base_value
              ...
              argument_values[responsive_variant][prop] = responsive_value.nil? || has_to_fallback ? definition.default_value(responsive_variant) : responsive_value
            end
      ...
      def normalize_argument_values!(arguments_definition:, argument_values: {}, fallback_to_default: true)
        arguments_definition.each do |prop, possible_definition|
          ...
          unless possible_definition.is_a? ArgumentDefinition
            nested_argument_values = argument_values[prop] || {}
            RESPONSIVE_VARIANTS.each do |variant|
              next unless argument_values.key? variant

              nested_argument_values[variant] = argument_values[variant][prop] if argument_values[variant].key? prop
            end

            argument_values[prop] = normalize_argument_values!(
              arguments_definition: possible_definition,
          ...
          definition = possible_definition
          is_base_value_set = argument_values.key?(prop)
          is_base_value_valid = definition.valid_value?(argument_values[prop])
          has_responsive_values = any_defined_responsive_values?(argument_values, prop)
          ...
          if definition.responsive?(:no) || (definition.responsive?(:transitional) && (is_base_value_set || definition.defined_default?) && !has_responsive_values)
            ...
            argument_values[prop] = definition.default_value if !is_base_value_set || has_to_fallback
            next
          end

          base_value = argument_values[prop] if is_base_value_set
          argument_values.delete(prop)
          ...
          RESPONSIVE_VARIANTS_MAP.each do |responsive_variant, responsive_variant_config|
            ...
            argument_values[responsive_variant][prop] = has_to_fallback || responsive_value.nil? ? definition.default_value(responsive_variant) : responsive_value
          end
        end
